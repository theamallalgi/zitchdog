...
local Util = require("zitchdog.util")
...
-- Retrieve the group function from the `groups` folder in your theme
function M.get_group(name)
	---@type {get: function, url: string}
	return Util.mod("zitchdog.groups." .. name)
end
...
function M.setup(colors, opts)
	...
	-- Enable all plugins if `all` is true
	...
	if not ret then
		ret = {}
		-- Merge plugin highlights into the final config
